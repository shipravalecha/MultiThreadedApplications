/**
 * This class performs the decay by applying weighting to the observations.
 */
...
public class HW6Decay 
{
    ...
    public static HeatMap[] weightedHeatMap;
    public static HeatMap[] reducedHeatmaps = new HeatMap[GeneralScan.INTERVAL];
    ...
    private static void animate() throws InterruptedException 
    {
        ...
        for (current = 0; current < weightedHeatMap.length; current++) 
        {
           fillGrid(grid);
            application.repaint();
			Thread.sleep(20);
    ...
    private static void fillGrid(Color[][] grid) 
    {
		for (int r = 0; r < grid.length; r++)
			for (int c = 0; c < grid[r].length; c++)
				grid[r][c] = interpolateColor(weightedHeatMap[current].data[r][c] / HOT_CALIB, COLD, HOT);
	}
    ...
    public static void main(String[] args) 
    {
        ...
        */
        ...
        // forkjoin pool to parallelize the tasks
        ...
        weightedHeatMap = new HeatMap[heatmapsByKey.length];

        for (int i = 0; i < heatmapsByKey.length; i++) 
        {
            weightedHeatMap[i] = new HeatMap(GeneralScan.DIM);
        }

        Thread[] threads = new Thread[GeneralScan.N_THREADS];
        
        reducedHeatmaps[0] = heatmapsByKey[0];

        for (int i = 1; i < GeneralScan.INTERVAL; i++) 
        {
            reducedHeatmaps[i] = HeatMap.add(heatmapsByKey[i], heatmapsByKey[i - 1]);
        }

        for (int i = 0; i < GeneralScan.N_THREADS; i++) 
        {
            Step8DecayTask task = new Step8DecayTask(i, heatmapsByKey);
